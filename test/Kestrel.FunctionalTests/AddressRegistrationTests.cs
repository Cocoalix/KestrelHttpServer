...
namespace Microsoft.AspNetCore.Server.Kestrel.FunctionalTests
{
    public class AddressRegistrationTests
    {
        ...
        private void ThrowsWhenBindingLocalhostToAddressInUse(AddressFamily addressFamily)
        {
            ...
            var thisAddress = addressFamily == AddressFamily.InterNetwork ? IPAddress.Loopback : IPAddress.IPv6Loopback;
            var otherAddress = addressFamily == AddressFamily.InterNetwork ? IPAddress.IPv6Loopback : IPAddress.Loopback;
            ...
            while (addressInUseCount < 10 && wrongMessageCount < 10)
            {
                ...
                {
                    socket.Bind(new IPEndPoint(thisAddress, 0));
                    socket.Listen(0);
                    var port = ((IPEndPoint)socket.LocalEndPoint).Port;

                    try
                    {
                        using (var otherSocket = new Socket(otherAddressFamily, SocketType.Stream, ProtocolType.Tcp))
                        {
                            otherSocket.Bind(new IPEndPoint(otherAddress, port));
                            otherSocket.Listen(0);
                        }
                    }
                    catch (SocketException)
                    {
                        // This address also being in use could lead to a startup exception with an unnexpected message,
                        // so try again with a new port.
                        addressInUseCount++;
                        continue;
                    }
                    ...
                    using (var host = hostBuilder.Build())
                    {
                        ...
                        var otherAddressString = $"http://{(otherAddressFamily == AddressFamily.InterNetwork ? "127.0.0.1" : "[::1]")}:{port}";
                        ...
                        if (exception.Message == CoreStrings.FormatEndpointAlreadyInUse(otherAddressString))
                        {
                            ...
                            // same port for the other address family between the binding check above and now.
                            wrongMessageCount++;
                            continue;
                        }
